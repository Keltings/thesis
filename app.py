...
# Data Exploration
    ...
    with col1:
        uploaded_file_2016 = st.file_uploader("Upload 2016 dataset (CSV)", type="csv", key="2016")
    with col2:
        uploaded_file_2021 = st.file_uploader("Upload 2021 dataset (CSV)", type="csv", key="2021")
    
    # If the files are uploaded
    if uploaded_file_2016 is not None and uploaded_file_2021 is not None:
        # Read the data
        ...
        if st.checkbox("Show Correlation between Features and Financial Exclusion"):
            ...
            for col in behavioral_features:
                if col in data_2021.columns:
                    data_2021[col] = data_2021[col].map({"yes": 1, "no": 0})
                    cols_to_analyze.append(col)
            
            # Add target
            data_2021["financially_excluded"] = data_2021["financially_excluded"].map({"yes": 1, "no": 0})
            
            # Calculate correlations
            corr_matrix = data_2021[cols_to_analyze + ["financially_excluded"]].corr()
        ...
        st.info("Please upload both 2016 and 2021 dataset files to explore the data.")
